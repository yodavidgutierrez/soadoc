...
(function (window, document, undefined) {
    (function (factory) {
        ...
        function _fnGetObjectDataFn(mSource) {
            if ($.isPlainObject(mSource)) {
                ...
                return function (data, type, extra) {
                    var t = o[type] || o._;
                    return t !== undefined ? t(data, type, extra) : data;
                };
            } else {
                if (mSource === null) {
        ...
        function _fnFeatureHtmlFilter(settings) {
            ...
            var jqFilter = $("input", filter).val(previousSearch.sSearch.replace('"', "&quot;")).bind("keyup.DT search.DT input.DT paste.DT cut.DT", _fnDataSource(settings) === "ssp" ? _fnThrottle(searchFn, 400) : searchFn).bind("keypress.DT", function (e) {
                if (e.keyCode == 13) {
                    return false;
                }
        ...
        function _fnCustomCleanText(uncleaned) {
            if (uncleaned != null) {
                var cleaned = uncleaned.toLowerCase();
                cleaned = cleaned.replace(/á/gi, "a");
                cleaned = cleaned.replace(/é/gi, "e");
                cleaned = cleaned.replace(/í/gi, "i");
                cleaned = cleaned.replace(/ó/gi, "o");
                cleaned = cleaned.replace(/ú/gi, "u");
        ...
        function _fnFilter(settings, input, force, regex, smart, caseInsensitive) {
            var cleanedInput = _fnCustomCleanText(input);
            var rpSearch = _fnFilterCreateSearch(cleanedInput, regex, smart, caseInsensitive);
            ...
            if (input.length <= 0) {
                ...
                for (i = display.length - 1; i >= 0; i--) {
                    var columnModifiedText = _fnCustomCleanText(settings.aoData[display[i]]._sFilterRow);
                    if (!rpSearch.test(columnModifiedText)) {
                        display.splice(i, 1);
                    }
                }
            }
        ...
        function _fnCalculateColumnWidths(oSettings) {
            ...
            if (!userInputs && !scrollX && !scrollY && columnCount == _fnVisbleColumns(oSettings) && columnCount == headerCells.length) {
                ...
                var tmpTable = $(table.cloneNode(false)).css("visibility", "hidden").removeAttr("id").append($(oSettings.nTHead).clone(false)).append($(oSettings.nTFoot).clone(false)).append($("<tbody><tr/></tbody>"));
        ...
        _Api.extend = function (scope, obj, ext) {
            ...
            for (i = 0, ien = ext.length; i < ien; i++) {
                struct = ext[i];
                obj[struct.name] = typeof struct.val === "function" ? methodScoping(struct.val, struct) : $.isPlainObject(struct.val) ? {} : struct.val;
            ...
            header: {
                _: function (settings, cell, column, classes) {
                    ...
                    $(settings.nTable).on("order.dt.DT", function (e, settings, sorting, columns) {
                        ...
                        cell.find("span").removeClass(classes.sSortJUIAsc + " " + classes.sSortJUIDesc + " " + classes.sSortJUI + " " + classes.sSortJUIAscAllowed + " " + classes.sSortJUIDescAllowed).addClass(columns[colIdx] == "asc" ? classes.sSortJUIAsc : columns[colIdx] == "desc" ? classes.sSortJUIDesc : column.sSortingClassJUI);
                    });
                }
            }
        });
            ...
            number: function (thousands, decimal, precision, prefix) {
                return {
                    display: function (d) {
                        ...
                        return (prefix || "") + intPart.toString().replace(/\B(?=(\d{3})+(?!\d))/g, thousands) + floatPart;
                    }
                };
            }
        };
